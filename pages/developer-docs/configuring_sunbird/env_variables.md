...
| 14    | telemetry_local_storage_type  | Indicate where user want to stroe telemetry | current storage type is kafka only                      |         kafka           | Sunbird-telemetry-service |
| 15    | telemetry_kafka_broker_list | ip address and ports of kafka | Kafka connection details. - <IP>:<port>,<IP>:<port>                    |   installer need to provide ip and port details  | Sunbird-telemetry-service |
| 16    | telemetry_kafka_topic  | Name of the kafka topic | suggested name topic name format is <env>.telemetry.ingestion                     |           | Sunbird-telemetry-service |
