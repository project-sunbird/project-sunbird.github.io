...
| 22 | sunbird_app_name  | This variable need to be set if adopter having mobile app. It will indicate name of the app. This variable is used during sending welcome sms to user. Welcome sms will come like this  Ex: If name is set<Name>, welcome to <environ>. You can login using your mobile number. Use <url> to login using browser, or download the <sunbird_app_name> app from Playstore , EX if name is not set <Name>, welcome to <environ>. You can login using your mobile number. Login at <url>|value will be shown on email |  Sunbird |Learner-Service |
| 23 | sunbird_registry_service_baseurl  | This variable is required to connect with Open-saber server.Open-saber is used to store user extra attribute. If the value is not set then user extra attribute won't be store insdie open-saber|To make open-saber call |   | Learner-Service | 
| 24 | sunbird_open_saber_bridge_enable  | This variable is required duing installation time, to indicate adopter needs open-saber or not. true indicate open-saber is required , fasle- not required. Incase of true user have to provide  sunbird_registry_service_baseurl env value|To enable or disable open-saber call | false  | Learner-Service | 
| 25 | sunbird_analytics_api_base_url  | This variable is required to decide content store call will happen local (inside sunbird) or global |To make analytics api call | <ekstep_api_base_url>/data/v3  | Learner-Service | 
| 26 | sunbird_search_service_api_base_url  | This variable is required to decide content store call will happen local (inside sunbird) or global |To make contnet search call | <ekstep_api_base_url>/search  | Learner-Service |  
