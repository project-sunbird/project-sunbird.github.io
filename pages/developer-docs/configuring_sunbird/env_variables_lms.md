...
| 24 | sunbird_open_saber_bridge_enable  | This variable is required during installation time, to indicate whether adopter needs open-saber | Values: True, to indicate if open-saber is required and false to indicate if not required. In case its true then the user must provide  sunbird_registry_service_baseurl env value. To enable or disable open-saber call, set to false | Learner-Service 
