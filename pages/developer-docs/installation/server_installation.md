...
6. Get the sunbird_sso_publickey from keycloak under **http://dns_name/auth -> realm settings -> keys -> public keys** (click on public keys) and paste the value obtained in variable **sunbird_sso_publickey** and provide a channel name **sunbird_default_channel** under config file and execute `./sunbird_install.sh -s core` to redeploy the core services.
...
9. Open http(s)://[domain-name] and verify your installation by entering the **username** with **username@channel** and **password** with the **password** given in config file.
